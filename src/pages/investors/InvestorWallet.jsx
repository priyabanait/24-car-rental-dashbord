...
export default function AddWalletAmount() {
    ...
    const handleAdminMessageSubmit = async (e) => {
      ...
      try {
        ...
        await axios.post('https://24-car-rental-backend.vercel.app/api/investor-wallet-message', {
  ...
  useEffect(() => {
    async function fetchInvestors() {
      setLoading(true);
      try {
        const res = await axios.get('https://24-car-rental-backend.vercel.app/api/investors?limit=1000');
  ...
  useEffect(() => {
    async function fetchWallet() {
      ...
      try {
        const investor = investors.find(inv => inv.id === selectedInvestor);
        if (investor) {
          const res = await axios.get(`https://24-car-rental-backend.vercel.app/api/investor-wallet/${investor.phone}`);
          setWallet(res.data);
        }
      } catch (err) {
        setWallet(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const investor = investors.find(inv => inv.id === selectedInvestor);
      await axios.post('https://24-car-rental-backend.vercel.app/api/investor-wallet', {
        phone: investor.phone,
