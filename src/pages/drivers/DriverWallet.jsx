...
export default function AddWalletAmount() {
  ...
  useEffect(() => {
    async function fetchDrivers() {
      setLoading(true);
      try {
        const res = await axios.get('https://24-car-rental-backend.vercel.app/api/drivers?limit=1000');
  ...
  useEffect(() => {
    async function fetchWallet() {
      ...
      try {
        ...
        if (driver) {
          const res = await axios.get(`https://24-car-rental-backend.vercel.app/api/driver-wallet/${driver.phone}`);
          setWallet(res.data);
        }
      } catch (err) {
        setWallet(null);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const driver = drivers.find(drv => drv._id === selectedDriver);
      await axios.post('https://24-car-rental-backend.vercel.app/api/driver-wallet', {
        phone: driver.phone,
  ...
  const handleAdminMessageSubmit = async (e) => {
    ...
    try {
      const driver = drivers.find(drv => drv._id === selectedDriver);
      await axios.post('https://24-car-rental-backend.vercel.app/api/driver-wallet-message', {
        phone: driver.phone,
