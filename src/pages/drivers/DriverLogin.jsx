...
const DriverAuth = () => {
  ...
  const handleSubmit = async (e) => {
      ...
      if (loginType === "password") {
        if (authMode === "signup") {
          if (!username || !mobile || !password) {
            setError("Please enter username, mobile number and password.");
            setLoading(false);
            return;
          }
    ...
    res = await fetch(`http://localhost:4000/api/drivers/signup`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ username, mobile, password }),
          });
        } else {
          // Login: only username and password required
          if (!username || !password) {
            setError("Please enter username and password.");
            setLoading(false);
            return;
          }
    res = await fetch(`http://localhost:4000/api/drivers/login`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (authMode === "signup") {
          ...
          res = await fetch(`http://localhost:4000/api/drivers/signup-otp`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ username, mobile, otp }),
          });
        } else {
          // Login with mobile and OTP (password)
          if (!mobile || !otp) {
            setError("Please enter mobile and OTP.");
            setLoading(false);
            return;
          }
          res = await fetch(`http://localhost:4000/api/drivers/login-otp`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
      ...
      <div className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl p-8 w-full max-w-md text-white border border-white/20">
        <form onSubmit={handleSubmit}>
          ...
          <h2 className="text-2xl font-semibold text-center mb-6">
            {authMode === "login" ? "Driver Login" : "Driver Signup"}
          </h2>
